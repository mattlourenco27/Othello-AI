...
// app window
Gtk::Window* glc::pWindow = nullptr;
...
// delete the window
void glc::on_quit_clicked() {
    if(pWindow) pWindow->hide(); //hide() will cause main::run() to end.
}

// finish construction of the window and connect signals
void glc::set_up(const Glib::RefPtr<Gtk::Builder> & refBuilder) {
    //Get the GtkBuilder-instantiated Window:
    refBuilder->get_widget("app_window", glc::pWindow);
    if(glc::pWindow) {
    //Get the GtkBuilder-instantiated Button, and connect a signal handler:
        Gtk::Button* pButton = nullptr;
        refBuilder->get_widget("quit_button", pButton);
        if(pButton) {
            pButton->signal_clicked().connect( sigc::ptr_fun(&glc::on_quit_clicked));
        }
    }
}
